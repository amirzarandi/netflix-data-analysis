...
## We use separate function  to split date added column into 2 columns

```{r}
netflix = netflix %>% separate(date_added,
                               c("date_added", "year_added"), sep = ",")
```

```{r}
netflix = netflix %>% separate(listed_in, 
                               c("Category1", "Category2", "Category3"), sep = ",")
```
...
```{r}
country %>% group_by(country) %>% count(country, type) %>% 
...
In the graph above, we see the density of movies and tv shows each year for Netflix. 

# Top 25 countries on Netflix

In the graph below we see the top 25 countries on Netflix.
...
netflix %>%
    ...
    labs(x="Sum of Movies and TV Shows Frequency", 
         y="Movies and TV Shows Rleased", title="Top 25 Countries on Netflix") +
    coord_flip() +
...
Here, we see that Netflix has the highes number of movies and TV shows rated TV-MA. 
...
netflix %>%
 ...
 ggplot() +
  ...
  theme_minimal() +
  facet_wrap(vars(type)) +theme(axis.text.x = 
...
The wordcloud above shows the most common words.